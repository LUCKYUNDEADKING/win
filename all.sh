...
log "${CYAN}▶▶▶ 一键安装：清理旧模块 → 安装 BBRPlus DKMS → NGINX 优化 ◀◀◀${NC}"
log "当前内核：$(uname -r)"
...
# 6. 添加 & 编译 & 安装
log "${GREEN}[6/9] 添加 & 编译 & 安装 DKMS 模块...${NC}"
...
# 9. 完成校验
log "${GREEN}[9/9] 检查结果...${NC}"
if sysctl net.ipv4.tcp_congestion_control | grep -q tcp_bbrplus; then
  log "${GREEN}✅ 安装成功：TCP拥塞控制 = tcp_bbrplus${NC}"
else
  log "${RED}❌ 安装失败：TCP拥塞控制仍为 $(sysctl net.ipv4.tcp_congestion_control | awk '{print \$3}')${NC}"
  exit 1
fi

log "${CYAN}建议重启后确保完全生效：sudo reboot${NC}"
EOF
